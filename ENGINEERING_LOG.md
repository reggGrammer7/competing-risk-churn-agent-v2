...
## Fixed: deployed backend crashed on Render's free tier with exit 137 (OOM), not a code bug

**Symptom:** Render logs showed `No open ports detected, continuing to
scan...` followed by `Exited with status 137`, in a crash loop -- the app
never successfully bound to `$PORT` at all.

**Diagnosis:** exit 137 is `128 + SIGKILL(9)` -- the Linux kernel killing
the process, almost always for exceeding a memory limit, not an
application-level error. `backend/rag/vector_retriever.py` loads
`sentence-transformers` (which pulls in PyTorch) at MODULE IMPORT time,
not lazily -- so merely importing `backend.rag.retriever` triggers a full
torch load immediately. Render's free tier has limited RAM, well below
what torch + sentence-transformers need just to import successfully. The
existing try/except fallback in `retriever.py` (see earlier entries) only
catches an import that *fails* (no internet, model not cached) -- it
cannot catch an OOM kill, because the OS terminates the whole process
outright; there's no Python exception to catch, because there's no Python
process left to raise one.

**The fix:** added a `DISABLE_VECTOR_RETRIEVER` environment variable,
checked in `backend/rag/retriever.py` BEFORE the vector_retriever import
is even attempted -- if set, it skips straight to the TF-IDF retriever,
so torch is never imported at all on a host where doing so is what kills
the process. Set to `"true"` by default in `render.yaml` for the free
tier. Verified locally by running the full test suite and a live app-boot
test with the env var set, confirming the app serves correctly on
TF-IDF-only mode with zero crashes.

**The broader lesson:** a try/except around a risky import only protects
against failures Python can actually observe and raise as an exception.
An OOM kill happens at the OS level, underneath the language runtime
entirely -- no amount of exception handling inside the killed process can
catch it. The only real fix is not doing the memory-heavy thing at all on
a host that can't afford it, which means the DECISION to skip it has to
happen before the risky import line is ever reached, not around it.
